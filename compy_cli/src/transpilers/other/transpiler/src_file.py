...
class SrcFileTranspiler:
    ...
    def transpile(self, file: Path):
        cpp_source, h_source, h_file = self._calc_cpp_and_h_source(file)
        self._write_cpp_file(file, cpp_source)
        self._write_h_file(h_file, h_source)

    def _calc_cpp_and_h_source(self, file: Path) -> tuple[str, str, Path]:
        py_src_file: Path = self._py_src_dir / file
        src_file_py_ast_tree: ast.Module = calc_ast(py_src_file)
        h_file: Path = file.with_suffix(".h")
        return *calc_src_file_cpp_and_h_source(
            src_file_py_ast_tree, h_file, self._maps, self._src_py_files
        ), h_file

    def _write_cpp_file(self, file: Path, cpp_source: str):
