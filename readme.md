...
no detectiable difference (of greater than 2%) compared to the identical C++ code I would write.
...
interpreter, but is also able to be transpiled to C++ and then built into an executable program.

Not all C++ features are supported, but enough that I care about are supported (or will be in future ComPy versions), so that I think I am content to use ComPy instead of C++.
I will give an idea about how you write code for ComPy projects and what the transpiled C++ code looks like in the section after the next section.

In the rest of this document I will give a brief idea about how to use ComPy and how ComPy works, as and introduction. Then before the v1.0.0 release, I will have complete documentation on a website that explains every detail possible so you can work with ComPy with a solid reference.
...
Then, you can run your generated executable manually, or you can use compy to run it with (if your executable is called 'main'):
...
order that they came in the parameters list, as done in the Greeter example
...
If the frozen=True was omitted, then the consts in the generated C++ struct goes away.

#### 6) Unions and Optionals
Unions and optionals are supported in ComPy. So if you are used to using
Pythons isinstance() function to check the type of an object, you can still
...
running the executable. But, there is nothing stopping you from running your code with the Python interpreter also, since the code you write is valid Python code. 
...
You can run with the Python interpetor with the command:
...
You can create ComPy compatiable libraries and upload them to PyPI to contribute to the ComPy ecosystem 
...
People like you creating ComPy libraries will be necessary to make ComPy as enjoyable to use as a typical 
programing language like Python, C++, Java, C#, or anything else is. This is because I likely don't have 
...
its likely much better to contribute by creating a ComPy library instead. You are free to do that without anyone
reviewing your work!
...
### Pure-libraries
Pure-libraries are libraries that are written with the ComPy framework. This is the easier one of the two library types but still very powerful.
You just write your ComPy code, transpile it to C++ (the generated C++ goes in a special folder), and then can upload
your library to PyPI so anyone can install it to their ComPy project with pip.

To setup a pure-library you run:

`compy init_pure_lib`

This will create the PyPI project structure for you with a pyproject.toml file, create your virtual environment, and install a few required libraries to the virtual environment.
...
A pure library is setup to be built with hatching (you can change that if you want):
...
After the v1.0.0 release of ComPy I plan to start making many bridge-libraries that I will need for my projects that I intent to use ComPy for (like a game engine).
...
the Python interpretor, the C++ code will be used by ComPy when the cmake project is being build, and the JSON files will tell ComPy how to transpile 
certain things. If that sounded confusing lets look at a quick example.

Lets say that you want to provide support for the Python 'time' standard library (or something effectively equivalent to it) within ComPy. You can create a bridge-library (lets call it "my_bridge_library" for the example) and add this Python code to it:
...
To setup a bridge-library you run:

`compy init_bridge_lib`

And again, a bridge library is setup to be built with hatching (you can change that if you want):

`python -m hatchling build`

## List of other details about writing ComPy code
- Tuples are transpiled to a PyTup type and I think are likely not performant with a large amount of elements. In ComPy tuples are meant to only store a small amount of elements.
...
- A big thing about accessing tuple elements and dict elements is you have to use special functions that I've called 'tg' and 'dg' (standing for tuple get and dict get). It is unfortunatly a little inconvenient, but something that I couldn't get a work around for. Its really only resulting in a couple extra characters for when you want to access tuple and dict elements.
- Quite a few string methods are supported but quite a few are not. I will add more string methods in future ComPy releases. Its just a matter of having the time to add them.
...
- 'global' and 'non local' is not supported
- enumerate, zip, reversed and list, set, and dict comprehensions are supported.

All other details I will provide when I write the docs.

## The bad (about ComPy)
ComPy will be rough around the edges. There will probably be lots of bugs at the beginning. Stablity will only improve with time.
...
- All sorts of libraries that you would expect in a good programming langauge (i.e. multi-threading/processing, JSON, high-quality file-interaction, os interactions, unittesting, etc.)
...
- You can write code that performs as well as C++ (the #1 most performant high-level language) with a Python syntax.
    - (If you find something in ComPy does not perform as well as something you could write in C++, please contact me with the details. I really want to identify these situations. My contact information is at the bottom.)
...
- The transpiler is very fast. Its execution time seems negligable compared to the cmake build time, so is not the bottleneck.
