...
def handle_with_item(nodes: list[ast.withitem],
                     ret_imports: set[CppInclude],
                     handle_expr,
                     ) -> str:
    error_str: str = "With statement can only be used as 'with open(arg1, ?optional_arg2) as name1'"
...
def _assert_with_item_is_open(nodes: list[ast.withitem], error_str: str) -> tuple[ast.withitem, list[ast.expr]]:
