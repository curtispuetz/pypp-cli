...
- Use the Python None. Instead you use compyOpt (optional)
- Print an optional
...
- Iterate over a tuple (in compy, tuples are just used as short data containers; lists, sets, or np arrays can be used 
...
- (tricky one) note that all returns in compy are by value, so therefore you should avoid returning large objects except when that large object is constructed in the function/method, 
...
functions/methods to return a reference, and a compy_ref() function to create a reference to a variable, but I won't right now, because I think it is not a pattern that a programming
