...
# Notes:
# - eventually, increasing the number of octaves becomes pointless if your resolution
# isn't that high
...
class PerlinNoise:
    def __init__(self, permutation_table_size: int, permuted_list: list[int]):
        self._p_len = permutation_table_size
        self._p = permuted_list

    def calc(
        self, x: float, y: float, octaves=1, persistence=0.5, lacunarity=2.0
    ) -> float:
        freq = 1
        amplitude = 1
        noise = 0
    ...
    def _calc_one_octave(self, x: float, y: float) -> float:
        ...
        dot1 = g00[0] * u + g00[1] * v
        dot2 = g10[0] * (u - 1) + g10[1] * v
        dot3 = g01[0] * u + g01[1] * (v - 1)
        dot4 = g11[0] * (u - 1) + g11[1] * (v - 1)

        i1 = _lerp(s, dot1, dot2)
        i2 = _lerp(s, dot3, dot4)
...
def create_perlin_noise(permutation_table_size: int, rng: random.Random) -> PerlinNoise:
    arr_to_shuffle: list[int] = [i for i in range(1, permutation_table_size + 1)]
    rng.shuffle(arr_to_shuffle)
    return PerlinNoise(permutation_table_size, arr_to_shuffle)
